...
\framesubtitle{}
\begin{algorithm}[H]
%	initialize $q_{\phi}(\theta| x)$ with K components \\
%	\tcp{if $q_{\phi}$ available by Algorithm~\ref*{alg:train_prop_prior}}
%	\tcp{initialize by replicating its}
...
%	\vspace{1px}
%	\caption{Training of posterior}
%	\label{alg:train_posterior}
\end{algorithm}
\end{frame} 
